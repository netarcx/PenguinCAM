# Automatic deployment to UP2

PenguinCAM deploys the latest **tested `main` commit** to the shop's UP2 server. The
`deploy-up2` job in `.github/workflows/integration.yaml` starts only after every unit,
system, feed-model, G-code audit, and JavaScript check succeeds for a push to `main`.
...
the `main` branch. Do not add a required reviewer if deployment should remain automatic.
...
2. The production environment admits only a successful `main` push.
