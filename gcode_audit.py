...
def audit_tube(name, face_width, tube_length, tube_height, square_end=True,
               cut_to_length=False, pockets=True, tool=0.157, wall=0.0625):
    ...
    pp.load_tube_pattern(face_width, tube_length, pockets=pockets)
...
def main():
    ...
    for face_width, height, label in ((1.0, 1.0, '1x1'), (2.0, 1.0, '2x1-flat'),
        ...
        for length in (6.0, 24.0):
            audit_tube(f'tube/{label}/{length:g}in', face_width, length, height)
    audit_tube('tube/2x1-flat/holes-only', 2.0, 24.0, 1.0, pockets=False)
